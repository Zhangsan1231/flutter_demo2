...
class DefaultRepositoryImpl extends BaseRepository implements DefaultRepository{
  @override
  Future<UserModel?> getInfo() {
   ...
   try {
     return callApiWithErrorParser(dioCall).then((response) => _parseInfoResponse(response));
   } catch (e) {
    //  logger.e('获取用户信息失败:$e');
     if( e is ApiException){
      rethrow;
     }else{
      throw ApiException(httpCode: -1, status: 'Parse Error',message: '数据解析失败');
     }
   }
  }
